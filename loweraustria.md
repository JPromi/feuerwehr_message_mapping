...
## REGEX für alle Felder
```regex
/^(?<type>[A-Za-z]+)(?<firedepartment_number>\d+)(?<group>[A-Za-z]+)\s+\((?<time>\d{1,2}:\d{2})\)\s+(?<alarm_type>[A-Za-z]+)(?<alarm_level>\d+)(?:\+(?<alarm_accessories>[^\s+]+(?:\+[^\s+]+)*))?\s+(?<alarm_text>.*?)\.\s+(?:(?<object>[^,:(]+?)(?:,\s*(?<street>[^,\d]+?)\s+(?<housenumber>\d[^,]*)\s*,\s*(?<zip>\d{4})\s+(?<city>[^:(]+?))?|(?<object_only>[^:(]+?)(?:\s*\((?<adress_additional>[^)]*)\))?)(?::\s*(?<note>.*?))?(?:\s*\((?<lat>[-\d.,]+)\s*,\s*(?<lng>[-\d.,]+)\))?\s*$/gm
```
